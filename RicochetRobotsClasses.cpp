...
void Board::displayRobotPostion()
{
	cout << endl << " 	Robots: " << theRobots [1] << " " << theRobots[2] << " " << theRobots[3] << " " << theRobots[4] << endl;
}
...
void addNodeToList(Node *&pHead, int moveNumber, Board theBoard)
{
	Node *pTemp;
	pTemp = new Node;
...
Node *deleteNodeFromList(Node *&pHead, int &moveNumber)
{
	if (pHead->pNext == NULL)
	{
...
int main()
{
    ...
    char EnterTrueOrFalse;
    cout << "enter T for True or F for false to indicate if boards and instructions will be displayed: ";
    cin >> EnterTrueOrFalse;
    EnterTrueOrFalse = toupper(EnterTrueOrFalse);
    
    if (EnterTrueOrFalse == 'T')
    {
    	displayIdentifyingInformationAndInstructions();
	}
	else if (EnterTrueOrFalse != 'T' && EnterTrueOrFalse != 'F')
	{
		cout << "*** Error. Please enter appropriate input. ***" << endl;
	...
	addNodeToList(pHead, moveNumber, theBoard);
	
	while (true)
	{
		cout << moveNumber << ". List: ";
		displayList(pHead);
		
		theBoard.displayRobotPostion();
		...
		else if (toupper(robot) == 'B')
		{
			deleteNodeFromList(pHead, moveNumber);
		...
		else
		{
			...
			moveRobotInIndicatedDirection( theBoard, robot, directionOffset, moveDirection, EnterTrueOrFalse);
			...
			if (EnterTrueOrFalse == 'T')
			{
				cout << endl << "Move robot " << theBoard.getGoalRobot() << " to square " << theBoard.getDestinationLetter() << endl;
				theBoard.display();
			}
		}
		
		if ((robot - '0' == theBoard.getGoalRobot()) && (theBoard.getRobotAt(robot - '0') == theBoard.getGoalPosition()))
		{
			cout << "Great Jobe! You did it in only " << moveNumber << " moves." << endl;
			break;
		}
